...
class Application:
	...
	self.c1=Checkbutton(frame,text="Time Reversal",bg="grey",variable=self.checkvar1,onvalue=1,offvalue=0,height=3,width=20)
	self.c1.pack(side=TOP)
	
	#Modulation
	self.checkvar2=IntVar()
	self.c2=Checkbutton(frame,text="Select for Modulation",bg="grey",variable=self.checkvar2,onvalue=1,offvalue=0,height=3,width=20)
	self.c2.pack(side=TOP)
	
	#Mixing
	self.checkvar3=IntVar()
	self.c3=Checkbutton(frame,text="Select for Mixing",bg="grey",variable=self.checkvar3,onvalue=1,offvalue=0,height=3,width=20)
	self.c3.pack(side=TOP)

	#Play Button
	self.play = Button(frame, bg="grey",text="Play", command=self.fin)
	self.play.pack(side=BOTTOM)

	#Play Button
	self.pause = Button(frame, bg="grey",text="Pause", command=self.pause)
    ...
    def time_shift(self):
		...
		if((self.varb.get()) > 0):
			if(self.type_channel == 1):
				a=[]
				for i in range(0,shift_frames):
					a.append(0)
				self.formated_data = a + self.formated_data
			else:
				a=[]
				for i in range(0,2*shift_frames):
					a.append(0)
				self.formated_data = a + self.formated_data
		else:
			if(self.type_channel == 1):
...
root=Tk()
root.geometry("1024x700+400+350")
...
mod=Button(root, text="Modulate And Play",fg="blue", command=modulate,bg="grey")
mod.place(relx=0.3,rely=0.92,anchor=CENTER)

#Mix AND Play Button
mix=Button(root,text="Mix And Play",fg="blue", command=mixing,bg="grey")
mix.place(relx=0.7,rely=0.92,anchor=CENTER)

#Record
rec=Button(root,text="Record",fg="blue",command=record,bg="grey")
rec.place(relx=0.5,rely=0.92,anchor=CENTER)

#Record's Play
rec=Button(root,text="Record's Play",fg="blue",command=rpl,bg="grey")
rec.place(relx=0.5,rely=0.96,anchor=CENTER)
